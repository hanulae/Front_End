...
const CallFormPage = () => {
  ...
  useFocusEffect(
    useCallback(() => {
      const fetchManagerPhoneNumber = async () => {
        ...
        if (info && info.data) {
          setManagerPhoneNumber(info.data.managerPhoneNumber);
        }
      };
  ...
  const managerPhone = usePhoneInput(managerPhoneNumber); // 초기값으로 managerPhoneNumber 설정
  ...
  // 출동 신청 훅
  const {loading, error, createManagerDispatchRequest} =
  ...
  const showConfirmDialog = () => {
    Alert.alert('출동신청', '해당 내용으로 출동신청하시겠습니까?', [
      {
        text: '출동신청',
      ...
      <View style={dynamicStyles.container}>
        ...
        <ScrollView
          ...
          <View style={dynamicStyles.bottomSection}>
            ...
            <Typo style={dynamicStyles.label}>
              상조 팀장 연락처{' '}
              <Typo
                fontSize={responsiveSizes.labelFontSize}
                style={dynamicStyles.required}>
